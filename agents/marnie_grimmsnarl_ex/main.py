"""Marnie's Grimmsnarl ex / Munkidori -- agente euristico a parametri.
...
VALUE_HOOK resta None: l'euristica ordina le mosse, una rete valutera' le
posizioni. Mai due sistemi che classificano la stessa cosa.
"""
...
#   DECK_LIST ha un assert di lunghezza all'import.
# - Cardinalita': _sanitize() su OGNI ritorno (unico punto di uscita di
#   _agent_impl); taglia a maxCount, riempie fino a minCount, scarta duplicati e
#   indici fuori range. minCount 0 resta 0 (declinare e' legale).
# - Mai solleva: agent() avvolge tutto in try/except BaseException; _fallback()
#   e' a sua volta protetto e degrada a [].
# - Budget: _STATE["spent"] accumula fra le chiamate; oltre _SOFT_DEADLINE
#   (480 s sui 600 disponibili) si salta ogni scoring e si torna la risposta
#   legale piu' economica.
# - Scope per-partita: _reset_if_new_match() azzera tempo e OpponentModel a
#   turn <= 1.
# - Import: solo stdlib (json, os, time, dataclasses) + cg.api.
# - Determinismo: nessun random; i pareggi si risolvono con sorted() stabile
#   (indice piu' basso) e con preferenze esplicite dove contava (attivo prima
#   della panchina, Stage 2 prima dello Stage 1).
...
#   TO_HAND, SWITCH/TO_ACTIVE, ACTIVATE -- piu' i contesti che questo mazzo
#   genera davvero: EVOLVE (Rare Candy), ATTACH_TO/ATTACH_FROM (Punk Up),
#   DAMAGE (rimbalzo Shadow Bullet), DAMAGE_COUNTER / REMOVE_DAMAGE_COUNTER(_COUNT)
#   (Adrena-Brain), DISCARD_ENERGY (costo di ritirata), DISCARD_TOOL_CARD,
#   DRAW_COUNT.
